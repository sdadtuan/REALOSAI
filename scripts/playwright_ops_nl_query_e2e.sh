#!/usr/bin/env bash
# RNOS-22 / AI-UC-016 — Playwright NL query smoke
set -euo pipefail
...
if ! curl -sf "${OPS_E2E_API_URL}/api/v1/ai/health" >/dev/null 2>&1; then
  ...
  (
    ...
    export PTT_STAFF_JWT_SECRET="${PTT_STAFF_JWT_SECRET:-rnos22-e2e-staff-jwt-secret-min-32-chars}"
    npm run start:prod
  ) >/tmp/rnos22-api.log 2>&1 &
  API_PID=$!
