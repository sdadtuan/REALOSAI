#!/usr/bin/env bash
# SCR-MOB-004 — Playwright CSKH board mobile card smoke
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"

ENV_FILE="${RNOS_MOB_CSKH_ENV:-$ROOT/deploy/env.local.example}"
if [[ -f "$ENV_FILE" ]]; then
  set -a
...
if ! curl -sf "${OPS_E2E_API_URL}/health" >/dev/null 2>&1; then
  ...
  (
    ...
    export PTT_AI_COPILOT_ENABLED=0
    export PTT_STAFF_ALLOW_STUB=1
    export PTT_STAFF_STUB_USERS="${OPS_E2E_STAFF_EMAIL}:${OPS_E2E_STAFF_PASSWORD}:1:1:E2E CSKH"
...
if [[ "${OPS_E2E_SKIP_SERVER:-0}" != "1" ]]; then
  if ! curl -sf "${OPS_E2E_URL}/login" >/dev/null 2>&1; then
    ...
    ) >/tmp/rnos-mob-cskh-ops-web.log 2>&1 &
    WEB_PID=$!
...
echo "==> Playwright SCR-MOB-004 CSKH board mobile"
(
  cd "$ROOT/services/ops-web"
  if [[ ! -d node_modules ]]; then npm ci; fi
