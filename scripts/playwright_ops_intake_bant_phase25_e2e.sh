#!/usr/bin/env bash
# INT-P25-18 — Playwright Funnel Stepper Phase 2.5 (Intake + Lead handoff)
set -euo pipefail
...
if [[ "${INT_P25_SKIP_BOOTSTRAP:-0}" != "1" ]]; then
  bash "$ROOT/scripts/intake_bant_phase25_e2e_bootstrap.sh"
fi

if ! curl -sf "${OPS_E2E_API_URL}/health" >/dev/null 2>&1; then
  echo "==> Start ptt-crm-api (presales on lead enabled)"
  (
    ...
    export PTT_STAFF_STUB_USERS="${OPS_E2E_STAFF_EMAIL}:${OPS_E2E_STAFF_PASSWORD}:1:1:E2E Intake"
...
echo "==> Playwright INT-P25-18 funnel stepper"
(
  cd "$ROOT/services/ops-web"
  if [[ ! -d node_modules ]]; then npm ci; fi
  npx playwright install chromium
  npm run test:e2e:intake-bant-phase25
)
