#!/usr/bin/env bash
# RNOS-21 — Playwright coach digest smoke
set -euo pipefail
...
if ! curl -sf "${OPS_E2E_API_URL}/api/v1/ai/health" >/dev/null 2>&1; then
  ...
  (
    ...
    export PTT_STAFF_STUB_USERS="${OPS_E2E_STAFF_EMAIL:-staff@demo.local}:${OPS_E2E_STAFF_PASSWORD:-demo12345}:1:1:E2E RNOS-21"
    export PTT_STAFF_JWT_SECRET="${PTT_STAFF_JWT_SECRET:-rnos21-e2e-staff-jwt-secret-min-32-chars}"
    npm run start:prod
  ) >/tmp/rnos21-api.log 2>&1 &
  API_PID=$!
