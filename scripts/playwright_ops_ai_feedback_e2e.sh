#!/usr/bin/env bash
# RNOS-29 — Playwright AI feedback loop smoke
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"

ENV_FILE="${RNOS29_ENV:-$ROOT/deploy/env.local.example}"
if [[ -f "$ENV_FILE" ]]; then
  set -a
...
_free_api_port_if_stale() {
  local code
  code="$(curl -s -o /dev/null -w '%{http_code}' "${OPS_E2E_API_URL}/api/v1/ai/analytics/acceptance" 2>/dev/null || echo 000)"
  ...
  if [[ -n "$pids" ]]; then
    echo "WARN  Stale Nest API on :$port (analytics HTTP $code) — stopping $pids"
...
if ! curl -sf "${OPS_E2E_API_URL}/api/v1/ai/health" >/dev/null 2>&1; then
  ...
  ANALYTICS_CODE="$(curl -s -o /dev/null -w '%{http_code}' "${OPS_E2E_API_URL}/api/v1/ai/analytics/acceptance" || echo 000)"
  if [[ "$ANALYTICS_CODE" == "404" || "$ANALYTICS_CODE" == "000" ]]; then
    echo "WARN  Analytics API not available (HTTP $ANALYTICS_CODE) — restarting Nest API"
    NEED_API_START=1
  else
    NEED_API_START=0
...
if [[ "${NEED_API_START:-0}" == "1" ]]; then
  _free_api_port_if_stale
  echo "==> Start ptt-crm-api (stub staff + AI copilot for E2E)"
  (
    ...
    export PTT_STAFF_STUB_USERS="${OPS_E2E_STAFF_EMAIL}:${OPS_E2E_STAFF_PASSWORD}:1:1:E2E RNOS29"
    export PTT_STAFF_JWT_SECRET="${PTT_STAFF_JWT_SECRET:-rnos29-e2e-staff-jwt-secret-min-32-chars}"
    export PTT_CRM_INTERNAL_KEY="${PTT_CRM_INTERNAL_KEY:-rnos29-e2e-internal-key}"
    npm run start:prod
  ) >/tmp/rnos29-api.log 2>&1 &
  API_PID=$!
...
echo "==> Playwright RNOS-29 AI feedback (ops-web via playwright webServer on ${OPS_E2E_URL})"
(
  cd "$ROOT/services/ops-web"
  if [[ ! -d node_modules ]]; then npm ci; fi
  npm run test:e2e:ai-feedback
)
